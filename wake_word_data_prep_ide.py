...
from data_generation_flow import data_generation_flow
from further_data_generation_flow import further_data_generation_flow
...
while True:
    cli_choice = input(dialog_handler_instance.render_template("input-numbered-main_choice"))

    if cli_choice == "1":
        # TODO: test this flow
       base_model_info = get_base_model_flow(source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name, wakeword_model_name)
    
    elif cli_choice == "2":
        #TODO: fix base_model_info, it doesn't show up in the flow
        if base_model_info is None:
            data_generation_flow(wakeword_model_name, source_directories, destination_directories, directories_to_gauss, pdsounds_directory, base_model_info=None)
        else:
            data_generation_flow(wakeword_model_name, source_directories, destination_directories, directories_to_gauss, pdsounds_directory, base_model_info)

    elif cli_choice == "3":
        further_data_generation_flow(directories_to_process, extra_audio_directories_labels, max_files_from_source_directory, max_files_per_destination_directory, wakeword_model_name)

    elif cli_choice == "4":
        get_base_model_flow(source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name, wakeword_model_name)
        data_generation_flow(wakeword_model_name, source_directories, destination_directories, directories_to_gauss, pdsounds_directory)
        further_data_generation_flow(directories_to_process, extra_audio_directories_labels, max_files_from_source_directory, max_files_per_destination_directory, wakeword_model_name)
        sys.exit()
