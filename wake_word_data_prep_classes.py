...
class BasicFileOperations:
    ...
    def convert_mp3s_in_directory_to_wavs(self, source_directory, destination_directory):
    ...
    def change_sample_rate_of_wav_file(self, file, source_directory, destination_directory):
    ...
    def change_sample_rate_of_wavs_in_directory(self, source_directory, destination_directory):
        print('This will also copy all files already in 16000 sample rate into the destination directory')
        files = self.get_files(source_directory)
        self.make_directory(destination_directory)
        for file in files: 
            converted = self.change_sample_rate_of_wav_file(file, source_directory, destination_directory)
            if not converted:
                self.copy_file(file, source_directory, destination_directory)

    def split_files_into_multiple_directories(self, files, number_of_files_per_directory, source_directory, destination_directory):
        ...
        for file in files:
            if file_count < number_of_files_per_directory:
                self.copy_file(file, source_directory, destination_directory + '_0' + str(directory_number))
                file_count += 1
...
class TrainTestSplit:
    ...
    def random_training_test_split(files, dataset_percent_size):
        random_selected_training_files = random.sample(files, int(len(files)*dataset_percent_size))
        random_selected_testing_files = [file for file in files if file not in random_selected_training_files]
    ...
    def split_directory(self, source_directory, training_directory, testing_directory, split_type):
        ...
        if split_type is 'random':
            random_selected_training_files, random_selected_testing_files = self.random_training_test_split(files, dataset_percent_size)
            basic_file_operations_instance.copy_directory(random_selected_training_files, source_directory, training_directory)
            basic_file_operations_instance.copy_directory(random_selected_testing_files, source_directory, testing_directory)
        if split_type is 'even_odd':
            random_selected_training_files, random_selected_testing_files = self.even_odd_training_test_split(files)
            basic_file_operations_instance.copy_directory(random_selected_training_files, source_directory, training_directory)
            basic_file_operations_instance.copy_directory(random_selected_testing_files, source_directory, testing_directory)
        if split_type is 'three_four':
            random_selected_training_files, random_selected_testing_files = self.three_four_training_test_split(files)
            basic_file_operations_instance.copy_directory(random_selected_training_files, source_directory, training_directory)
            basic_file_operations_instance.copy_directory(random_selected_testing_files, source_directory, testing_directory)
   
    def split_multiple_directories(self, source_directory, destination_directory, random_split_directories, even_odd_split_directories, three_four_split_directories):
        sub_directories = random_split_directories + even_odd_split_directories + three_four_split_directories
        for sub_directory in sub_directories:
            ...
            for random_split_directory in random_split_directories:
                if sub_directory is random_split_directory:
                    split_type = 'random'
                    self.split_directory(source_directory+sub_directory, training_directory, testing_directory, split_type)
    ...
    def experimental_splits(self, source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name):
        ...
        for model in model_names:
            ...
            self.split_multiple_directories(source_directory, destination_directory, random_split_directories, even_odd_split_directories, three_four_split_directories)
        return model_names
    ...
    def split_incremental_results(self, model_name):
        ...
        for source_directory in source_directories:
            basic_file_operations_instance.copy_directory(random_selected_training_files, source_directory, training_directory)
            basic_file_operations_instance.copy_directory(random_selected_testing_files, source_directory, testing_directory)
        print('Finished spliting all files in generated test and training to random test and training directories')
...
class PreciseModelingOperations:
    ...
    def run_precise_train(model_name, epochs=None, source_directory=None):
        ...
        if epochs is None:
            ...
            training_output = subprocess.Popen(['precise-train', '-e', '50','-s', '0.215', '-b', '100',  '-sb', model_name + '.net', source_directory], stdout=subprocess.PIPE)
        else:
            training_output = subprocess.Popen(['precise-train', '-e', epochs,'-s', '0.215', '-b', '100', '-sb', model_name + '.net', source_directory], stdout=subprocess.PIPE)
    ...
    def get_models_analytics(self):
        ...
        return statistics.mean(val_acc_list), statistics.stdev(val_acc_list), statistics.mean(acc_list), statistics.stdev(acc_list)
    ...
    def get_optimal_training_model_analytics(self):
        ...
        average_acc_for_min_loss_models, stdev_acc_for_min_loss_models, average_val_acc_for_min_loss_models, stdev_val_acc_for_min_loss_models, average_acc_for_min_val_loss_models, stdev_acc_for_min_val_loss_models, average_val_acc_for_min_val_loss_models, stdev_val_acc_for_min_val_loss_models = self.model_analytics_instance.get_model_analytics()
        return average_acc_for_min_loss_models, stdev_acc_for_min_loss_models, average_val_acc_for_min_loss_models, stdev_val_acc_for_min_loss_models
    ...
    def rename_model(self, model_name, selected_model_name):
        ...
        for extension in model_extensions:
            ...
            if extension is '.logs':
                basic_file_operations_instance.rename_directory(file_to_rename + '/', new_file_name + '/')
            else:
                basic_file_operations_instance.rename_file(file_to_rename, new_file_name)
    ...
    def incremental_training(self, model_name, incremental_data_directory):
        ...
        training_output = subprocess.Popen(['precise-train-incremental', temporary_model_name + '.net', source_directory, '-r', incremental_data_directory], stdout=subprocess.PIPE)
        stdout = training_output.communicate()
        return stdout[0].decode('utf-8').split('\n')
    ...
    def add_background_noise(model_name, noise_directory):
        ...
        noise_generation_output = subprocess.Popen(['precise-add-noise', '-if', '5' , model_directory, noise_directory, destination_directory], stdout=subprocess.PIPE)
    ...
    def move_noise_directories(self, model_name, source_directories, destination_directories):
        ...
        for source, destination in zip(source_directories, destination_directories):
            ...
            #TODO TEST THIS
        basic_file_operations_instance.delete_directory(model_directory + 'background_noise/')
...
class GaussianNoiseHandler:
    ...
    def add_gaussian_noise(self, file, directory):
        basic_file_operations_instance = BasicFileOperations()
        sample_frequency, wave_data = basic_file_operations_instance.read_wave_file(directory + file)
        ...
        for noise_level in [15, 30, 50, 60]:
            ...
            basic_file_operations_instance.write_wave_file(gauss_directory + gauss_file_name, sample_frequency, noisy_data)
