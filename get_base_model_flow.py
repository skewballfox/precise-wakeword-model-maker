from wake_word_data_prep_classes import BasicFileOperations, TrainTestSplit, PreciseModelingOperations
import os
...
def move_random_user_collection(source_directory):
    ...
    if not os.path.exists(random_destination_directory):
        ...
        basic_file_operations_instance.copy_directory(files, random_source_directory, random_destination_directory)
        basic_file_operations_instance.delete_files_in_directory(files, random_source_directory)
    
    return random_destination_directory

def split_data_and_get_best_model_flow(source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name):
    print(dialog_handler_instance.render_template("inform-splitting_data", source_directory=source_directory))
    ...
    average_acc_for_min_loss_models, stdev_acc_for_min_loss_models, average_val_acc_for_min_loss_models, stdev_val_acc_for_min_loss_models = precise_modeling_operations_instance.get_optimal_training_model_analytics()
    ...
    experimental_average_accuracy_dialog = dialog_handler_instance.render_template("inform-accuracy", average_val_acc=average_val_acc_for_min_loss_models, standard_deviation_val_acc=stdev_val_acc_for_min_loss_models, average_acc=average_acc_for_min_loss_models, standard_deviation_acc=stdev_acc_for_min_loss_models)

    print(experimental_average_accuracy_dialog)
    
    best_training_set_accuracy_model_name, best_training_set_accuracy_model = precise_modeling_operations_instance.get_best_loss_model()
    ...
    experimental_average_accuracy_dialog = dialog_handler_instance.render_template("inform-best_model", selected_model_name=selected_model_name, selected_model_results=selected_model_results)

    return (selected_model_name, selected_model_results, dataset_size, experimental_average_accuracy_dialog)

def train_model_flow(wakeword_model_name, epochs=None):
    print(dialog_handler_instance.render_template("inform-training_start", wakeword_model_name=wakeword_model_name))
    training_run = precise_modeling_operations_instance.run_precise_train(wakeword_model_name, epochs)
    print(dialog_handler_instance.render_template("inform-training_complete", wakeword_model_name=wakeword_model_name))
    
    precise_modeling_operations_instance.get_model_info(wakeword_model_name, training_run)

    return precise_modeling_operations_instance.models[wakeword_model_name]

def incremental_training_flow(random_user_recordings_directory, selected_model_name, epochs=None):
    #TODO: add CLI elements into class?
    print(dialog_handler_instance.render_template("inform-incremental_training_start", random_user_recordings_directory=random_user_recordings_directory))
    precise_modeling_operations_instance.incremental_training(selected_model_name, random_user_recordings_directory)
    print(dialog_handler_instance.render_template("inform-incremental_training_complete", random_user_recordings_directory=random_user_recordings_directory))
...
def get_base_model_flow(source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name, wakeword_model_name):
    random_user_recordings_directory = move_random_user_collection(source_directory)
    
    selected_model_name, selected_model_results, selected_model_dataset_size, experimental_average_accuracy_dialog = split_data_and_get_best_model_flow(source_directory, random_split_directories, even_odd_split_directories, three_four_split_directories, root_model_name)
    
    precise_modeling_operations_instance.delete_experiment_directories(selected_model_name)
    
    base_model_info = incremental_training_flow(random_user_recordings_directory, selected_model_name, epochs='50')
    
    precise_modeling_operations_instance.delete_experiment_models(selected_model_name)
    precise_modeling_operations_instance.rename_model(selected_model_name, wakeword_model_name)
    basic_file_operations_instance.rename_directory(selected_model_name, wakeword_model_name)
    print(dialog_handler_instance.render_template("inform-changed_model_name", selected_model_name=selected_model_name, wakeword_model_name=wakeword_model_name))
    
    print(dialog_handler_instance.render_template("inform-base_model_accuracies"))
    print(experimental_average_accuracy_dialog)
   
    print(dialog_handler_instance.render_template("inform-original_best_base_model_results", selected_model_name=selected_model_name, selected_model_results=selected_model_results, dataset_size=selected_model_dataset_size))
    print(f'Current base model with incremental training on your random audio recordings:\n {wakeword_model_name}: {base_model_info[0]} \n dataset size: {base_model_info[2]}')
    print(f'Not bad when you consider the average accuracies of the first base model...')
    
    print(dialog_handler_instance.render_template("inform-continue", wakeword_model_name=wakeword_model_name))
