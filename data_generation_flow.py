from wake_word_data_prep_classes import BasicFileOperations, PreciseModelingOperations, GaussianNoiseHandler
...
def data_generation_flow(wakeword_model_name, source_directories, destination_directories, directories_to_gauss, noise_directory, base_model_info=None):
    if noise_directory is None:
        noise_directory = str(input(f'Enter the relative noise files directory path (ie {wakeword_model_name}/pdsounds_march2009/mp3/):'))
    noise_destination_directory = wakeword_model_name + '/random/non-utterances/pdsounds_march2009/'
    basic_file_operations_instance.convert_mp3s_in_directory_to_wavs(noise_directory, noise_destination_directory)
    
    print('Adding background noise...')
    precise_modeling_operations_instance.add_background_noise(wakeword_model_name, noise_destination_directory)
